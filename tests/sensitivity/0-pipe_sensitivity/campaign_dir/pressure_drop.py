...
def main(path):
    ...
    inlet_slice = threshold.slice(
        origin=[voxeldata.bounds[0] + dx, 0, 0], normal=[1, 0, 0]
    )
    outlet_slice = threshold.slice(
        origin=[voxeldata.bounds[1] - dx, 0, 0], normal=[1, 0, 0]
    )
    ...
    dataframe = pd.DataFrame(
        {
            "inlet_VFR": [inlet["VFR"][0]],
            "outlet_VFR": [outlet["VFR"][0]],
