...
void writeInhomogenHDF5(MultiBlockLattice3D<T,DESCRIPTOR>& lattice, const SimPar &sim, plint iter, string outDir, MultiNTensorField3D<T> *field1,MultiNTensorField3D<T> *field2)
{
                ...
                for(unsigned int k = LocalBulk.getBulk().z0; k <= LocalBulk.getBulk().z1; k++){
                    ...
                    if (field1 != nullptr) {
                        ...
                        Field1.push_back(0.0);
                                        ...
                                        if (field2 != nullptr) {
                        double foundField2 = *field2->getComponent(blockId).get(LocalX, LocalY, LocalZ);
                        Field2.push_back(foundField2); 
                    }
                    else {
                        Field2.push_back(0.0);
                    } 
                    // Rank of current mpi thread
                    this_rank.push_back(RankID);
    ...
    DataSet Field1_data = file.createDataSet<float>("Field1", DataSpace(Dims), props);
    // Field2
    DataSet Field2_data = file.createDataSet<float>("Field2", DataSpace(Dims), props);
    ...
    Field1_data.select(ElementSet(GlobalID)).write(Field1, xfer_props);
    // Field2
    Field1_data.select(ElementSet(GlobalID)).write(Field2, xfer_props);
    // MPI Rank
    Rank.select(ElementSet(GlobalID)).write(this_rank, xfer_props);
